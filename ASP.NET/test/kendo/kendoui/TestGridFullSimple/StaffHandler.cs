    ...
    public class StaffHandler : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            switch (context.Request.HttpMethod)
            {
                case "GET":
                    {
                        if (!string.IsNullOrEmpty(skipStr = context.Request.QueryString["skip"])
                            && !string.IsNullOrEmpty(takeStr = context.Request.QueryString["take"]))
                        {
                            int.TryParse(skipStr, out skip);
                            int.TryParse(takeStr, out take);

                            List<Staff>
                                staffsPage = getStaffsPage(staffs, skip, take);
                ...
                case "DELETE":
                    {
                        ...
                        switch (context.Request.HttpMethod)
                        {
                            ...
                            case "PUT":
                                {
                                    List<Staff>
                                        updatedStaffs = updateStaffs(staffs, cudStaffs);

                                    staffResponse = new StaffResponse
                                    {
                                        success = true,
                                        staff = updatedStaffs,
                                        total = updatedStaffs.AsEnumerable().Count()
                                    };
        ...
        List<Staff> getStaffsPage(DataTable dataTable, int skip, int take)
        {
            return dataTable.AsEnumerable().Skip(skip).Take(take).Select(r => new Staff
                {
                    Id = r.Field<long>("Id"),
        ...
        List<Staff> updateStaffs(DataTable dataTable, List<Staff> staffs)
        {
            staffs.ForEach(staff =>
            {
                DataRow
                ...
                if ((tmpDataRow = dataTable.Rows.Find(staff.Id)) != null)
                {
                    ...
                    if (tmpDataRow.IsNull("Salary") || Convert.ToDecimal(tmpDataRow["Salary"]) == 0)
                        tmpDataRow["Salary"] = 10;

                    staff.Salary = Convert.ToDecimal(tmpDataRow["Salary"])*4;
                }
            });
