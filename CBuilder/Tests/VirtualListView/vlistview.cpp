...
__fastcall TfrmVirtualLView::TfrmVirtualLView(TComponent* Owner)
        : TForm(Owner)
{
   AnsiString FileName=ExtractFilePath(Application->ExeName)+"Ons.log";
      ...
      TListItem *Item)
{
        ...
        if(File.good())
          File<<"ListView OnData Item->Index="<<Item->Index<<std::endl;
                        ...
                        if (!(IsFolder(FIShellFolder, ShellItem->ID)))
                           {
                                ...
                                if(File.good())
                                  File<<"ListView OnData SubItem->Add().1"<<std::endl;
                           }
                        else
                           {
                                Item->SubItems->Add("");
                                if(File.good())
                                  File<<"ListView OnData SubItem->Add().2"<<std::endl;
                           }

                        if(File.good())
                          File<<"ListView OnData SubItem->Add().3"<<std::endl;
...
void __fastcall TfrmVirtualLView::FormClose(TObject *Sender,
      TCloseAction &Action)
{
   File.close();
}
