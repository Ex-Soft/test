        ...
        if(POP3IdLogDebugToFileCheckBox->Checked)
          {
             ...
             LogFile=new TFileStream(FileName,fmCreate|fmOpenReadWrite|fmShareDenyWrite);
          }

        IdLogDebugPOP3->Active=IdLogDebugCheckBox->Checked;

        IdPOP3->Host=POP3HostEdit->Text.Trim();
        IdPOP3->Username=POP3UserIDEdit->Text.Trim();
...
void __fastcall TMainForm::IdLogDebugPOP3Connect(TIdConnectionIntercept *ASender)
{
   POP3StatusWindowMemo->Lines->Add("IdLogDebugPOP3 OnConnect");
}
//---------------------------------------------------------------------------

void __fastcall TMainForm::IdLogDebugPOP3Disconnect(TIdConnectionIntercept *ASender)
{
   POP3StatusWindowMemo->Lines->Add("IdLogDebugPOP3 OnDisconnect");
}
//---------------------------------------------------------------------------


void __fastcall TMainForm::IdLogDebugPOP3Receive(TIdConnectionIntercept *ASender, TStream *AStream)
{
   POP3StatusWindowMemo->Lines->Add("IdLogDebugPOP3 OnReceive");
   POP3StatusWindowMemo->Lines->Add("AStream->Size="+IntToStr(AStream->Size));
   POP3StatusWindowMemo->Lines->Add("AStream->Position="+IntToStr(AStream->Position));
   ...
   if(Buff)
     {
        do
         {
            Count = BuffSize<AStream->Size ? BuffSize : AStream->Size;
            AStream->ReadBuffer(Buff,Count);
            *(Buff+Count)='\x0';
            if(*(Buff+Count-2)=='\r' && *(Buff+Count-1)=='\n')
              *(Buff+Count-2)='\x0';
            POP3StatusWindowMemo->Lines->Add(Buff);
   ...
   if(LogFile)
     {
        LogFile->CopyFrom(AStream,AStream->Size);
        AStream->Position=0;
     }
   ...
   tmpStringList->LoadFromStream(AStream);
   POP3StatusWindowMemo->Lines->AddStrings(tmpStringList);
}
//---------------------------------------------------------------------------

void __fastcall TMainForm::IdLogDebugPOP3Send(TIdConnectionIntercept *ASender, TStream *AStream)
{
   POP3StatusWindowMemo->Lines->Add("IdLogDebugPOP3 OnSend");
   POP3StatusWindowMemo->Lines->Add("AStream->Size="+IntToStr(AStream->Size));
   POP3StatusWindowMemo->Lines->Add("AStream->Position="+IntToStr(AStream->Position));
                             ...
                             if(Attachment=dynamic_cast<TIdAttachment *>(IdMessagePOP3->MessageParts->Items[i]))
                               {
                                  ...
                                  S=IncludeTrailingPathDelimiter(POP3AttachmentPathEdit->Text.Trim())+Attachment->FileName;
                                  if(FileExists(S))
                                    DeleteFile(S);
                                  Attachment->SaveToFile(S);
                               }
                          }
