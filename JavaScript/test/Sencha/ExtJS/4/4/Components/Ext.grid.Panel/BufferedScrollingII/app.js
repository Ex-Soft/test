...
*/
	var
		numberFieldId = new Ext.form.field.Number({
			allowDecimals: false,
			minValue: 0,
			hideTrigger: true
		}),
		grid = Ext.create("Ext.grid.Panel", {
			...
			dockedItems: [{
					...
					numberFieldId,
				{
					...
					handler: function(btn, e) {
						...
						if((g=btn.up("grid"))
							&& (r=g.getStore().getById(numberFieldId.getValue()))
							&& window.console
							&& console.log)
								console.log("%o", r);
					}
			...
			listeners: {
				afterrender: function(grid, eOpts) {
					if(window.console && console.log)
						console.log("%i Ext.grid.Panel.afterrender(%o)", arguments.length, arguments);

					grid.getStore().guaranteeRange(0, pageSize-1);
				}
			}
		});
